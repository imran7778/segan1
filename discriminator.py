...
def discriminator(self, wave_in, reuse=False):
        ...
        with tf.variable_scope('d_model') as scope:
            ...
            for block_idx, fmaps in enumerate(self.d_num_fmaps):
                ...
                print(f'D block {block_idx} output shape: {hi.get_shape()}')
                if not reuse:
                    print()
            if not reuse:
                print('discriminator deconved shape: ', hi.get_shape())
            hi_shape = hi.get_shape().as_list()
            hi_f = tf.reshape(hi, [batch_size, -1])  # Use tf.reshape to flatten the tensor
            ...
            d_logit_out = tf.reshape(d_logit_out, [batch_size, -1])  # Ensure shape is known before Dense layer
