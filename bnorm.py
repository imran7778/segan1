...
class VBN(object):
    ...
    def __init__(self, x, name, epsilon=1e-5):
        ...
        with tf.variable_scope(name) as scope:
            ...
            self.batch_size = tf.shape(x)[0]  # Changed: Use tf.shape to handle dynamic shapes
    ...
    def __call__(self, x):
        ...
        with tf.variable_scope(self.name) as scope:
            new_coeff = 1. / (tf.cast(self.batch_size, tf.float32) + 1.)  # Changed: Use tf.cast for dynamic shape
